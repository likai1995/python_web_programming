...
def main3():
    """
    非阻塞式I/O
    将socket套接字设置为非阻塞，以轮询的方式不断询问内核数据是否准备就绪
    """
    socket_server = socket(AF_INET, SOCK_STREAM)
    socket_server.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
    #将套接字设置为非阻塞
    ...
    while True:
        try:
            ...
            conn_list.append(new_socket)
            addr_list[new_socket] = client_info
        ...
        for conn in conn_list:
            try:
                data = conn.recv(1024)
                if data:
                    print(f"收到来自{addr_list[conn]}的数据：{data.decode()}")
                    conn.send(data)
                else:
                    print('连接{}已断开'.format(addr_list[conn]))
                    conn.close()
                ...
                try:
                    ...
                    while raw_data:
                        if raw_data == 'q':
                            ...
                            print(f"收到来自{addr_list[sock]}的数据：{raw_data.decode()}")
                            sock.send(raw_data)
                            raw_data = sock.recv(1024)
                    print('客户端{}已断开'.format(addr_list[sock]))
                    sock.close()
